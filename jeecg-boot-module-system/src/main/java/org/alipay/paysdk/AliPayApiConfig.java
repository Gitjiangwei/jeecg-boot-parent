...
@Data
@Component
@PropertySource(value = "classpath:alipayConfig/alipay.properties")
public class AliPayApiConfig implements Serializable {
    ...
    @Value("${gatewayUrl}")
    private String serviceUrl; //支付宝请求网关
    @Value("${charset}")
    private String charset; //字符编码格式
    @Value("${sign_type}")
    private String signType; //签名方式
    @Value("${format}")
    private String format; //格式化方式

    private boolean certModel;
    private String appCertPath; //应用公钥证书 (证书模式必须)
    private String aliPayCertPath; //支付宝公钥证书 (证书模式必须)
    private String aliPayRootCertPath; // 支付宝根证书 (证书模式必须)
